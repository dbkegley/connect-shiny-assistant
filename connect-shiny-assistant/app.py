...
def server(input: Inputs, output: Outputs, session: Session):
    ...
    if os.path.exists(SHINY_APP_DIR):
        start_content()
        shiny_panel_visible_smooth_transition.set(False)
    ...
    async def _():
        ...
        messages = transform_messages_to_chatlas_content_format(messages)
        response = await model.stream_async(messages, echo="all")
...
#         try:
#             response_stream = await model.stream_async(messages)
...
#         await chat.append_message_stream(logging_stream_wrapper())
    ...
    async def transform_response(content: str, chunk: str, done: bool) -> str:
        print("TRANSFORMAATIONING THE ASSISTANT RESPONSES")
        # if done:
    ...
    async def _send_shinyapp_code():
        ...
        #     "set-shiny-content", {"files": files_in_shinyapp_tags()}
        # )
...
def transform_messages_to_chatlas_content_format(
    ...
    for msg in reversed(messages):
        # TODO: Does it matter that this is ignoring the roles?
        c = msg["content"]
